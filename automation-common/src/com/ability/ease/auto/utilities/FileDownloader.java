...
/*    public String fileDownloader(WebElement element) throws Exception {
    ...
    public String downloader(String downloadLocation) throws Exception {
        ...
        if (downloadLocation.trim().equals("")) {
            throw new Exception("The download location you have specified does not link to anything!");
        }
        ...
        //client.setState(mimicCookieState(driver.manage().getCookieNamed("JSESSIONID")));
        ...
        File f = new File("summaryreport.pdf");
        FileOutputStream downloadFile = new FileOutputStream(f);
        try {
            int status = client.executeMethod(getRequest);
            report.report("HTTP Status {} when getting '{}'"+ status+ downloadURL.toExternalForm());
            ...
            while ((bytes = in.read(block, offset, len)) > -1) {
            	downloadFile.write(block, 0, bytes);
            }
            downloadFile.close();
            in.close();
            report.report("File downloaded to '{}'"+ f.getAbsolutePath(),ReportAttribute.BOLD);
        } catch (Exception Ex) {
            report.report("Download failed: {}"+ Ex, Reporter.WARNING);
