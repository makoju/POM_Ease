...
public class ClaimsPageVersion2 extends AbstractPageObject {
	...
	String searchResPageHdr = elementprop
			.getProperty("ADVANCE_SEARCH_RESULTS_PAGE_HEADER");
	...
	public boolean verifyEditClaimLineOptions(String HIC, String claimLineToEdit)
		...
		String renchIconXpath = MessageFormat.format(
				elementprop.getProperty("RENCH_ICON_XPATH"), "'" + HIC + "'");
		String editIconXpath = MessageFormat.format(
				elementprop.getProperty("EDIT_CLAIM_LINE_XPATH"), "'ub42_"
						+ claimLineToEdit + "'");
		...
		if (waitForElementToBeClickable(ByLocator.xpath, searchResPageHdr, 15) != null) {
			WebElement renchIcon = waitForElementToBeClickable(ByLocator.xpath,
					renchIconXpath, 5);
			if (renchIcon != null) {
				renchIcon.click();
				WebElement lockIcon = waitForElementToBeClickable(
						ByLocator.name, ub04LockIcon, 15);
				moveToElement(lockIcon);
				safeJavaScriptClick(ub04LockResubmit);
				helper.moveToEditIcon(editIconXpath);
				editMenuList = driver.findElements(By.xpath(elementprop
						.getProperty("EDIT_MENU_OPTIONS_XPATH")));
	...
	public void navigateToPage() throws Exception {
		WebElement mydde = waitForElementToBeClickable(ByLocator.linktext,
				myDDELink, 30);
		String classAttr = mydde.getAttribute("class");
		if (mydde != null) {
			if (!classAttr
					.equalsIgnoreCase("topNavAnchor topNavAnchorSelected")) {
				safeJavaScriptClick(myDDELink);
				WebElement ub04 = waitForElementToBeClickable(ByLocator.id,
						ub04Link, 30);
				if (ub04 != null) {
